...
## 0. ~~紧急修复：Timeout 缓冲泄漏 + 重复 attach + "Nothing suspended." 空等~~ ✅ 已完成 (v0.7.2)

> 2026-06-26 新增。覆盖用户报告的 4 个场景。已实施并发布。
> 额外发现并修复第 5 个 bug：en_US locale 千位逗号导致行号截断（`line=3,956` → `3`）。

### Bug A: Timeout 后 reader 缓冲未清空（场景 2 & 3 核心根因）

**根因**：`reader.rs` 的 `read_until_prompt()` 超时路径返回 `self.text.clone()` 但**不清空**
`self.text`/`self.raw`。后续命令在旧缓冲上叠加 → 正则匹配到错误的事件 banner / 行号 / 旧 prompt。

**修复**：超时路径改用 `self.take_text()`（同时清空 raw 与 text）。

### Bug B: Normal 命令仅 state=Suspended 时 purge（加重 Bug A）

**根因**：`session.rs:execute()` 只在 `mode==Normal && state==Suspended` 时 `purge_pending()`。
超时后 state=Running → 后续 Normal 命令跳过 purge → channel 中迟到字节污染下一次读取。

**修复**：Normal 命令始终 purge（`purge_pending` 是 `try_recv` 非阻塞，开销可忽略）。

### Bug C: attach 不检测重复 target（场景 1，且间接触发 2&3）

**根因**：`manager.rs:create_attach()` 无去重。两 jdb 连同一端口互相干扰（kill 发 resume 解冻对方）。

**修复**：attach 前扫描存活 session 是否已有相同 target，有则拒绝并提示复用/先 kill。
新增 `Error::DuplicateTarget` 变体。

### Bug D: Blocking 模式收到 "Nothing suspended." 后空等超时（场景 2 序幕）

**根因**：attach suspend=n 的 VM 后 state 设 Suspended（实际 VM 在跑）。`cont` 让 jdb 回
"Nothing suspended.\n> \n"，Blocking 模式忽略裸 prompt，空等 30s 才 timeout。

**修复**：在 `try_match` 的 Blocking + 裸 prompt 分支中检测 prompt 前文本含 "Nothing suspended"
则视为立即完成（VM 未挂起，cont 是空操作）。同时 `map_outcome` 将此情况下的 state 置为 Running
（反映真实 VM 状态）。
...
| 文件 | 改动 |
|------|------|
| `src/jdb/reader.rs` | Timeout take_text；Blocking "Nothing suspended." 快速返回 |
| `src/session.rs` | purge_pending 去掉 state 条件 |
| `src/daemon/manager.rs` | create_attach 去重检查 |
| `src/error.rs` | 新增 DuplicateTarget 变体 |
| 单元测试 | reader: timeout 后缓冲空；"Nothing suspended." 快速返回；manager: 重复 attach 被拒 |
...
1. **#0（紧急修复）** — 本次实施。修复 Timeout 缓冲泄漏 + 重复 attach + "Nothing suspended." 空等。
2. **#3（健康探测+编码）** — 风险最低、独立、直接改善体验（attach 探测已在 #0 中完成）。
3. **#1（suspend policy）** — 性价比高但**需先做 jdb 可行性实测**，确认能力边界后再定方案。
4. **#2（sentinel 重构）** — 最重要但最危险，单独立项，先攒 transcript 测试集再动 reader。
