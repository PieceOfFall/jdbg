...
- JDI `inspect` keeps safe field-reading semantics and does not invoke getters: it shares the JavaParser expression grammar with `print`/`eval` (locals, `this`, instance/static fields, field chains, array access) but runs in a no-invoke mode that rejects method calls with `method_invocation_not_allowed`, pointing the caller to `print`/`eval`. JDI `print`, `eval`, `dump`, `set`, and `force-return` are executable capabilities: they may invoke target methods and mutate state. `setValue` evaluates `<lvalue> = <value>` semantics for locals, fields, and array elements. `force-return` evaluates the value expression and calls `ThreadReference.forceEarlyReturn`; void force return is explicitly unsupported for now.
