...
fn build_command(cli: &Cli) -> anyhow::Result<Command> {
    let cmd = match &cli.command {
        ...
        Commands::Threads { filter } => Command::Threads { filter: filter.clone() },
        ...
        Commands::Set { lvalue, value } => Command::Set { lvalue: lvalue.clone(), value: value.clone() },
        Commands::Ignore { exception, mode } => Command::Ignore { exception: exception.clone(), mode: mode.clone() },
