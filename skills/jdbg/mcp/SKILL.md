...
JDI `step`/`next`/`step_out` remain on the currently stopped thread. Concurrent events from unrelated threads
are resumed while the step is active and cannot replace its result.
